...
$(document).ready(function () {
    ...
    var table = $('#TableId').DataTable(
    {
        ...
        "ajax":
            {
                "url": "/Utilities/GetuExternalTicketData?id=Answered",
                "type": "POST",
                "dataType": "JSON"
            },
        "columns": [
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row, meta) {
                            //if (type === 'display') {
                            data = '<a class="btn btn-default btn-sm" target="_blank" href="/Utilities/ExtChat/' + row.ticketRef + '" id="startChat" title="' + row.ticketRef + '"><i class="fa fa-comments-o"></i></a>';
                            //}
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row, meta) {
                            //if (type === 'display') {
                            data = '<a class="btn btn-default btn-sm" id="uClose" title="' + row.CaseRef + '"><i class="fa fa-file-word-o"></i></a>';
                            //}
    ...
    $('#TableId tbody').on('click', '#uClose', function (e) {
        e.preventDefault();


        var postData;
        postData = { id: $(this).attr('title') };

        $.ajax({
            type: 'POST',
            url: '/Utilities/closeUTicket',
            dataType: 'json',
            data: postData,
            traditional: true,
            success: function (data) {
                onAjaxRequestSuccess(data);
            },
            error: function (data) { },
        ...
        var onAjaxRequestSuccess = function (result) {
            if (result.EnableError) {
                $(".alarrt").fadeIn(2000).show();
                // Setting.
            } else if (result.EnableSuccess) {
                $(".succex").fadeIn(2000).show();
                table.draw();
            }
        }
