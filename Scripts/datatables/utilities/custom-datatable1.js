...
$(document).ready(function () {
    ...
    var table = $('#TableId').DataTable(
    {
        ...
        "ajax":
            {
                "url": "/Utilities/GetExternalTicketData/Unanswered",
                "type": "POST",
                "dataType": "JSON"
            },
        "columns": [
                    ...
                    { "data": "Priority" },
                    {
                        ...
                        "render": function (data, type, row, meta) {
                            //if (type === 'display') {
                            data = '<a class="btn btn-default btn-sm" id="getViewPdf" title="' + row.caseRef + '.pdf"><i class="fa fa-file-pdf-o"></i></a>';
                            //}
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row, meta) {
                            //if (type === 'display') {
                            data = '<a class="btn btn-default btn-sm" id="cApprove" title="' + row.ticketRef + '"><i class="fa fa-check"></i></a>';
                            //}
    ...
    $('#TableId tbody').on('click', '#cApprove', function (e) {
        var appID2 = $(this).attr('title');
        var postData = { tRef: appID2 };
        $.ajax({
            type: 'POST',
            url: '/Utilities/receiveTicket',
            dataType: 'json',
            data: postData,
            traditional: true,
        ...
        var onAjaxRequestSuccess = function (result) {
            if (result.EnableError) {
                $(".alarrt").fadeIn(1000).show();
            } else if (result.EnableSuccess) {
                table.draw();
                $(".succex").fadeIn(1000).show();
            }
        }
    });
