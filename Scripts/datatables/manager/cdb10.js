...
function getData(aa, ab, ac, ad, ae, af, ag, ah, clearStatus, bStat) {
    ...
    $.ajax({
        type: "GET",
        url: "/api/BackEnd/GetDataItem_TopUps",
        contentType: "application/json; charset=utf-8",
        data: { offset: aa, typ: ab, param: ac, param2: ad, dateFrom: ae, dateTo: af, dateSingle: ag, TopUpType: ah },
        dataType: "json",
        success: function (result) {
            ...
            $('.trendingItems').empty();
            $('#data-item-template').tmpl(result.iTopUps).appendTo('.trendingItems');
            $('.page-link[ogNum="' + result.demandNo + '"]').addClass('pglActiv');

            console.log(result.iTopUps[0].Title);
        },
        complete: function () {
        },
...
$(function () {
    ...
    $("#addTopUp").click(function (event) {
        ...
        TopUp.Title = $('#Title').val();
        TopUp.Address = $('#Address').val();
        TopUp.State = $('#State').val();
        TopUp.ZoneType = $('#ZoneType').val();
        TopUp.City = $('#City').val();

        $.ajax({
            type: "POST",
            url: "/api/ApiUtilities/addTopUp",
            data: TopUp,
            dataType: "json",
            success: function (result) {
                modal.dialog('close');
