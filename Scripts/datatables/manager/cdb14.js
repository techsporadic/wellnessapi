...
function getData(aa, ab, ac, ad, ae, af, ag, ah, clearStatus, bStat) {
    ...
    $.ajax({
        type: "GET",
        url: "/api/BackEnd/GetDataItem_Payments",
        contentType: "application/json; charset=utf-8",
        data: { offset: aa, typ: ab, param: ac, param2: ad, dateFrom: ae, dateTo: af, dateSingle: ag, PaymentType: ah },
        dataType: "json",
        success: function (result) {
            modal.dialog('close');

            if (result.iPayments.Length < 1) {
                $(".mainDataBody").hide();
                $(".auxDataBody").show();
            }
                ...
                $('.trendingItems').empty();
                $('#data-item-template').tmpl(result.iPayments).appendTo('.trendingItems');
