...
function getRealTime(dateval) {
    var dd = dateval;
    var orderdate = new Date(dd);
    return orderdate.toString("hh:mm tt");
}
...
function getData(aa, ab, ac, ad, ae, af, ag, ah, clearStatus, bStat) {
    ...
    $.ajax({
        type: "GET",
        url: "/api/ApiUtilities/GetDataItem_Schedules",
        contentType: "application/json; charset=utf-8",
        data: { offset: aa, typ: ab, param: ac, param2: ad, dateFrom: ae, dateTo: af, dateSingle: ag, ScheduleType: ah },
        dataType: "json",
        success: function (result) {
            modal.dialog('close');

            if (result.iSchedules.Length < 1) {
                $(".mainDataBody").hide();
                $(".auxDataBody").show();
            }
                ...
                $('.trendingItems').empty();
                $('#data-item-template').tmpl(result.iSchedules).appendTo('.trendingItems');
...
$(function () {
    ...
    $(document).on("click", ".editRecord", function () {
        ...
        var zon = $(this);
        var dtt = new Date(zon.attr('scheduledate'));



        var date1 = new Date(zon.attr('timefrom'));
        var options = {
            hour: 'numeric',
            minute: 'numeric',
            hour12: false
        };
        var timeString = date1.toLocaleString('en-US', options);

        var date2 = new Date(zon.attr('timeto'));
        var timeString1 = date2.toLocaleString('en-US', options);

        $("#ubdAjaxK #mTitle").val(zon.attr('title'));
        $("#ubdAjaxK #mScheduleType").val(zon.attr('scheduletype')); 
        $("#ubdAjaxK #mScheduleDate").val(getRealDate2(dtt));
        $("#ubdAjaxK #mTimeFrom").val(timeString.toString());
        $("#ubdAjaxK #mTimeTo").val(timeString1.toString());
        $("#ubdAjaxK #mId").val(zon.attr('id'));
    ...
    $(document).on("click", ".deleteSchedule", function () {
        ...
        if (confirm("Do you want to delete this ?"))
        {
            $.ajax({
                type: "GET",
                url: "/api/ApiUtilities/deleteSchedule?id=" + schid,
                data: {},
                dataType: "json",
                success: function (result) {
                    modal.dialog('close');
    ...
    $("#ubdAjaxH").submit(function (event) {
        ...
        getData(dCount, rVali, rValii, rValiii, $("#dateFrom").val(), $("#dateTo").val(), $("#dateSingle").val(), $("#nScheduleType").val());
    ...
    $("#addSchedule").click(function (event) {
        ...
        newSchedule.Title = $('#Title').val();
        newSchedule.ScheduleType = $('#ScheduleType').val();
        newSchedule.ScheduleDate = $('#ScheduleDate').val();
        newSchedule.DateTimeFrom = $('#DateTimeFrom').val();
        newSchedule.DateTimeTo = $('#DateTimeTo').val();

        $.ajax({
            type: "POST",
            url: "/api/ApiUtilities/addSchedule",
            data: newSchedule,
            dataType: "json",
            success: function (result) {
                modal.dialog('close');
    ...
    $("#modifySchedule").click(function (event) {
        event.preventDefault();
        var modal = $('<div>').dialog({ modal: true });
        modal.dialog('widget').hide();
        ...
        var newSchedule = new Object();
        newSchedule.Title = $('#mTitle').val();
        newSchedule.ScheduleType = $('#mScheduleType').val();
        newSchedule.ScheduleDate = $('#mScheduleDate').val();
        newSchedule.TimeFrom = $('#mTimeFrom').val();
        newSchedule.TimeTo = $('#mTimeTo').val();
        newSchedule.Id = $('#mId').val();

        $.ajax({
            type: "POST",
            url: "/api/ApiUtilities/modifySchedule",
            data: newSchedule,
            dataType: "json",
            success: function (result) {
                modal.dialog('close');
