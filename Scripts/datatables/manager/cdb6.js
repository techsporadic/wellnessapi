...
function getData(bk, aa, ab, ac, ad, ae, af, ag, ah, clearStatus, bStat) {
    ...
    $.ajax({
        type: "GET",
        url: "/api/Middle/GetDataItem_Units",
        contentType: "application/json; charset=utf-8",
        data: { blockid: bk, offset: aa, typ: ab, param: ac, param2: ad, dateFrom: ae, dateTo: af, dateSingle: ag, ZoneType: ah },
        dataType: "json",
...
function getRecords(bidd) {
    var modal = $('<div>').dialog({ modal: true });
    modal.dialog('widget').hide();

    var cdata = { eid: bidd, scope: "Block" };
        ...
        success: function (result) {
            ...
            if (result.iFees.length > 0) {
                ...
                result.iFees.forEach(function (item) {
                    ...
                    //}
                    $("#FeeList").append("<tr><td>" + truncateString(item.Title, 12) + "</td><td>" + item.FeeType + "</td><td>" + item.Frequency + "</td><td>" + humanise(item.CollectedEvery) + "</td><td>" + addCommas(item.FeeAmount) + "</td><td>" + item.Scope + "</td><td>" + getRealDate(item.NextDueDate) + "</td><td><a class='noColor editFee' href='#' data-toggle='modal' data-target='#newModifyFees' CollectedEvery='" + item.CollectedEvery + "' Description='" + item.Description + "' xId='" + item.Id + "' ElementId='" + item.ElementId + "' Scope='" + item.Scope + "' Title='" + item.Title + "' FeeAmount='" + item.FeeAmount + "' FeeType='" + item.FeeType + "' Frequency='" + item.Frequency + "' NextDueDate='" + getRealDate2(item.NextDueDate) + "' PercentagePenalty='" + item.PercentagePenalty + "' PenaltyEvery='" + item.PenaltyEvery + "' OriginalAmount='" + item.OriginalAmount + "' PenalizeOriginal='" + item.PenalizeOriginal + "'><i class='fas fa-pencil-alt'></i></a></td></tr>");
...
$(function () {
    ...
    var dCount = 1;
    var dataString;
    bid = getCookieValue("block_id");
    $("#rbid").val(bid);

    var joinQ = ""; var rVali = "active", rValii = 10, rValiii = "";

    getData(bid, dCount, rVali, rValii, rValiii, $("#dateFrom").val(), $("#dateTo").val(), $("#dateSingle").val(), "");
    getRecords(bid);
    ...
    $(document).on("click", "li .pgLink", function (e) {
        ...
        getData(bid, dCount, rVali, rValii, rValiii, $("#dateFrom").val(), $("#dateTo").val(), $("#dateSingle").val(), "", $(this).attr("id"));
    ...
    $(document).on("click", ".reportIssue", function () {
        ...
        $("#ubdAjaxK #LIScopeTitle").val(zon.attr('unittitle'));
    ...
    $("#addUnit").click(function (event) {
        ...
        var unit = new Object();
        unit.Title = $('#Title').val();
        unit.BlockId = $('#BlockId').val();
        unit.ZoneId = $('#ZoneId').val();

        $.ajax({
            type: "POST",
            url: "/api/Middle/addUnit",
            data: unit,
            dataType: "json",
            success: function (result) {
                modal.dialog('close');
        ...
        var onAjaxRequestSuccess = function (result) {
            if (result.EnableError) {
                ...
                getData(bid, 1, "active", 10, "", $("#dateFrom").val(), $("#dateTo").val(), $("#dateSingle").val(), "");
                mySnack();
            }
        }
    });

    $("#addManyUnitBtn").click(function (event) {
        event.preventDefault();
        ...
        var numUnits = $('#numValue').val();

        $.ajax({
            type: "GET",
            url: "/api/ApiResources/createUnits?num=" + numUnits + "&bid=" + bid,
            data: {},
            dataType: "json",
            success: function (result) {
                modal.dialog('close');
        ...
        var onAjaxRequestSuccess = function (result) {
            if (result.EnableError) {
                $("#errorInfoCreateUnit").html(result.ErrorMsg);
            } else if (result.EnableSuccess) {
                $("#addManyUnitModal").modal('hide');
                getData(bid, 1, "active", 10, "", $("#dateFrom").val(), $("#dateTo").val(), $("#dateSingle").val(), "");
            }
        }
    });

    $("#modifyUnit").click(function (event) {
        event.preventDefault();
        var modal = $('<div>').dialog({ modal: true });
        modal.dialog('widget').hide();
        ...
        var unit = new Object();
        unit.Title = $('#uTitle').val();
        unit.BlockId = $('#BlockId').val();
        unit.Id = $('#uId').val();

        $.ajax({
            type: "POST",
            url: "/api/ApiUtilities/modifyUnit",
            data: unit,
            dataType: "json",
            success: function (result) {
                modal.dialog('close');
