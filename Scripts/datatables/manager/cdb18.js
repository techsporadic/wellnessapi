...
function getData(aa, ab, ac, ad, ae, af, ag, ah, clearStatus, bStat) {
    ...
    $.ajax({
        type: "GET",
        url: "/api/BackEnd/GetDataItem_WithdrawalRequests",
        contentType: "application/json; charset=utf-8",
        data: { offset: aa, typ: ab, param: ac, param2: ad, dateFrom: ae, dateTo: af, dateSingle: ag, RequestType: ah },
        dataType: "json",
        success: function (result) {
            modal.dialog('close');

            if (result.iRequests.Length < 1) {
                $(".mainDataBody").hide();
                $(".auxDataBody").show();
            }
                ...
                $('.trendingItems').empty();
                $('#data-item-template').tmpl(result.iRequests).appendTo('.trendingItems');
...
$(function () {
    ...
    $("#ubdAjaxH").submit(function (event) {
        ...
        getData(dCount, rVali, rValii, rValiii, $("#dateFrom").val(), $("#dateTo").val(), $("#dateSingle").val(), $("#RequestType").val());
